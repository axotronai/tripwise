...
export async function GET(_: NextRequest, { params }: Params) {
  ...
  const { data, error } = await admin.from('hotels').select('*').eq('trip_id', id).not('day_id', 'is', null)
  if (error) return NextResponse.json({ error: error.message }, { status: 500 })

  const dayHotels: Record<string, { id: string; name: string; city: string; type: string; price_per_night: number; rating: number; amenities: string[] }> = {}
