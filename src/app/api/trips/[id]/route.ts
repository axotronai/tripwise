...
export async function GET(_: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  const daysWithSorted = (days || []).map(day => ({
    ...day,
    activities: (day.activities || []).sort((a: { order_index: number }, b: { order_index: number }) => a.order_index - b.order_index),
  }))
...
export async function PATCH(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  const allowed = ['total_budget', 'travel_style', 'group_size', 'children', 'title']
  const update: Record<string, unknown> = {}
  for (const key of allowed) {
    if (body[key] !== undefined) update[key] = body[key]
  }

  // Use admin client — trip ownership already enforced by the client knowing the trip ID
